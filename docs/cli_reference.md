...
Rows inside `off_target/results/*/analysis.tsv` and the aggregated `combined_offtargets.tsv` include a `species` column so you can filter hits directly. The aggregated table additionally carries the four-way classification per alignment: `hit_class` is exactly one of `on_target`, `ortholog`, `repeat` or `off_target`, `matched_symbol` is the resolved gene symbol (the literal `unknown` when the transcript index has none, so a thin annotation is visible rather than silent), and `symbol_lookup_missing` flags the rows where the ortholog check could not run. Only `hit_class = off_target` rows are liabilities, and they are the same population the per-candidate `off_target_count` is derived from — the row-level and candidate-level views cannot disagree. Aggregated summaries collapse those values into `human` vs `other` buckets, exposing `hits_per_species`, `human_hits`, and `other_species_hits` in `combined_summary.json` plus the workflow console output. The workflow also records the resolved reference decision in `logs/workflow_summary.json` (`reference_summary.transcriptome`) so each run documents whether the transcriptome reference was disabled, defaulted, or explicitly provided.
