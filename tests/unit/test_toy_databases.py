...
from sirnaforge.core.off_target import build_bwa_index, build_bowtie_index
...
@pytest.mark.smoke
@pytest.mark.skipif(not Path("/usr/bin/bwa-mem2").exists() and not Path("/usr/local/bin/bwa-mem2").exists(), 
...
@pytest.mark.smoke
@pytest.mark.skipif(not Path("/usr/bin/bowtie-build").exists() and not Path("/usr/local/bin/bowtie-build").exists(), 
...
def test_toy_mirna_bowtie_index_building():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Check that index files were created
        index_files = [f"{index_prefix}.{ext}" for ext in ["1.ebwt", "2.ebwt", "3.ebwt", "4.ebwt", "rev.1.ebwt", "rev.2.ebwt"]]
        for index_file in index_files:
