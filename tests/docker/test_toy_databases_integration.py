...
from sirnaforge.core.off_target import OffTargetAnalysisManager, build_bwa_index, build_bowtie_index
...
def test_toy_transcriptome_analysis_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager = OffTargetAnalysisManager(
            species="toy_human", 
            transcriptome_index=index_prefix
        )
        
        output_prefix = str(temp_path / "test_output")
        tsv_path, json_path = manager.analyze_transcriptome_off_targets(
            str(candidate_file), 
            output_prefix
        )
...
def test_toy_mirna_analysis_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager = OffTargetAnalysisManager(
            species="toy_human", 
            mirna_index=index_prefix
        )
        
        output_prefix = str(temp_path / "test_output")
        tsv_path, json_path = manager.analyze_mirna_off_targets(
            str(candidate_file), 
            output_prefix
        )
