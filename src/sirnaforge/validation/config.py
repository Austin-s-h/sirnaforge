...
class ValidationConfig(BaseModel):
    ...
    default_level: ValidationLevel = Field(
        ...
        description="Default validation level for all stages"
    )
    
    # Stage-specific validation levels
    stage_levels: Dict[ValidationStage, ValidationLevel] = Field(
        default_factory=dict,
        description="Per-stage validation level overrides"
    )
    
    # Validation options
    validate_sequences: bool = Field(
        default=True,
        description="Enable nucleotide sequence validation"
    )
    
    validate_ranges: bool = Field(
        default=True,
        description="Enable numerical range validation"
    )
    
    validate_consistency: bool = Field(
        default=True,
        description="Enable cross-field consistency validation"
    )
    
    validate_biology: bool = Field(
        default=True,
        description="Enable bioinformatics-specific validation"
    )
    ...
    collect_all_errors: bool = Field(
        default=False,
        description="Collect all validation errors instead of failing fast"
    )
    ...
    batch_size: int = Field(
        default=1000,
        ge=1,
        description="Batch size for large dataset validation"
    )
...
class ValidationPresets:
    ...
    def production() -> ValidationConfig:
        ...
        return ValidationConfig(
            default_level=ValidationLevel.STRICT,
