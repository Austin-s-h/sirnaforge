...
class ValidationMiddleware:
        ...
        for transcript in transcripts:
            ...
            if not transcript.sequence:
                ...
                if self.config.validate_sequences:
                    seq_result = ValidationUtils.validate_nucleotide_sequence(
                        transcript.sequence, allow_ambiguous=True
                    )
                    result.merge(seq_result)
                
                # Validate sequence length
                if len(transcript.sequence) < 100:
                    result.add_warning(f"Transcript {transcript.transcript_id} is very short ({len(transcript.sequence)} nt)")
    ...
    def validate_transcript_id_consistency(
        ...
        consistency_result = ValidationUtils.validate_transcript_ids_consistency(
            candidate_ids, orf_ids, transcript_ids
        )
    ...
    def _handle_validation_result(self, stage: ValidationStage, report: ValidationReport) -> None:
        ...
        if report.overall_result.warnings:
            logger.warning(f"Validation warnings for stage {stage.value}: {len(report.overall_result.warnings)} warnings")
