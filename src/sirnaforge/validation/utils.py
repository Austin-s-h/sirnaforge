...
class ValidationUtils:
    ...
    def validate_parameter_consistency(params: DesignParameters) -> ValidationResult:
        ...
        total_weight = (
            params.scoring.asymmetry + params.scoring.gc_content + 
            params.scoring.accessibility + params.scoring.off_target + 
    ...
    def validate_candidate_consistency(candidate: SiRNACandidate) -> ValidationResult:
        ...
        if len(candidate.guide_sequence) != len(candidate.passenger_sequence):
            result.add_error("Guide and passenger sequences have different lengths")
        ...
        if abs(calculated_gc - candidate.gc_content) > 5.0:
            result.add_warning(f"Reported GC content ({candidate.gc_content:.1f}%) "
                              f"differs from calculated ({calculated_gc:.1f}%)")
    ...
    def validate_dataframe_schema(df: pd.DataFrame, schema_type: str) -> ValidationResult:
        ...
        try:
            if schema_type == "sirna_candidates":
                ...
                result.add_metadata("validated_rows", len(validated_df))
                ...
                validated_df = OffTargetHitsSchema.validate(df)
                result.add_metadata("validated_rows", len(validated_df))
